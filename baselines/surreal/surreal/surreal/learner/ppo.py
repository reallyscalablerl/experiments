...
class PPOSubprocLearner:
    def __init__(self, rank, learner_config, env_config, session_config):
        # GPU setting
        self.rank = rank
        ...
        self.gpu_option = f'cuda:{rank}'
        self.use_cuda = True

        self.context = zmq.Context()
        self.socket = self.context.socket(zmq.REP)
        self.socket.connect(f"tcp://localhost:{8877 + self.rank + 1}")

        dist.init_process_group('nccl', world_size=SURREAL_WORLD_SIZE, rank=rank, init_method='tcp://localhost:8877')
        print(f"DDP learner {self.rank} has been initialized!")
    ...
    def _clip_loss(self, obs, actions, advantages, behave_pol): 
        ...
        # assert not torch.any(torch.isnan(learn_pol)), (obs, self.cells)
        learn_prob = self.pd.likelihood(actions, learn_pol)
        behave_prob = self.pd.likelihood(actions, behave_pol)
        prob_ratio = learn_prob / (behave_prob + 1e-8)
        # assert not torch.any(torch.isnan(obs))
        # assert not torch.any(torch.isnan(learn_prob))
        # assert not torch.any(torch.isnan(behave_prob))
        # assert not torch.any(torch.isnan(prob_ratio))
        cliped_ratio = torch.clamp(prob_ratio, 1 - self.clip_epsilon, 
                                               1 + self.clip_epsilon)
        # assert not torch.any(torch.isnan(advantages))
        surr = -prob_ratio * advantages.view(-1, 1)
        cliped_surr = -cliped_ratio * advantages.view(-1, 1)
        # assert not torch.any(torch.isnan(surr))
        # assert not torch.any(torch.isnan(cliped_surr))
    ...
    def _value_loss(self, obs, returns):
        ...
        assert not torch.any(torch.isnan(returns))
    ...
    def _optimize(self, obs, actions, rewards, obs_next, persistent_infos, onetime_infos, dones):
        ...
        with tx.device_scope(self.gpu_option):
            ...
            stats['_avg_log_sig'] = self.model.actor.module.log_var.mean().item()
...
def main(rank, batch_channel, stat_channel, learner_config, env_config, session_config):
    l = PPOSubprocLearner(rank, learner_config, env_config, session_config)
    l._setup_prefetching()
    while True:
        if l.exp_counter >= l.learner_config.parameter_publish.exp_interval:
            l._post_publish()
        torch.distributed.barrier()
        batch = l._prefetch_queue.get()
        # print(f"recv data rank {l.rank}")
        # batch = pickle.loads(l.socket.recv())
        # batch = batch_channel.get(timeout=0.1)
        batch = l._preprocess_batch_ppo(batch)
        torch.distributed.barrier()
        # print(f"before optimize {l.rank}")
        _ = l._optimize(
            batch.obs,
            batch.actions,
            batch.rewards,
        ...
        # print(f"after optimize {l.rank}")
        torch.distributed.barrier()
        l.exp_counter += l.batch_size
        # print(l.rank, "before put stat")
        stat_channel.put(1)
        # print(l.rank, "after put stat")
        # l.socket.send(pickle.dumps(tensorplex_update_dict))
        # stat_channel.put_nowait(tensorplex_update_dict)
...
class PPOLearner(Learner):
    ...
    def __init__(self, learner_config, env_config, session_config):
        ...
        self.stat_queues = [mp.Queue(32) for _ in range(SURREAL_WORLD_SIZE)]
        self.batch_queues = [mp.Queue(1) for _ in range(SURREAL_WORLD_SIZE)]

        self.context = zmq.Context()
        self.sockets = [self.context.socket(zmq.REQ) for _ in range(SURREAL_WORLD_SIZE)]
        for i, socket in enumerate(self.sockets):
            socket.bind(f"tcp://*:{8878 + i}")

        self.subproc_learners = [mp.Process(target=main, kwargs=dict(rank=i,batch_channel=self.batch_queues[i],
         stat_channel=self.stat_queues[i], learner_config=learner_config, 
         env_config=env_config, session_config=session_config)) for i in range(SURREAL_WORLD_SIZE)]
        for p in self.subproc_learners:
            p.start()
    ...
    def learn(self):
        def recursive_apply(x, fn):
            if isinstance(x, dict):
                return type(x)({k: recursive_apply(v, fn) for k, v in x.items()})
            elif isinstance(x, list):
                return type(x)([fn(xx) for xx in x])
            else:
                return fn(x)
        
        def recursive_print(x, prefix=""):
            if isinstance(x, dict):
                for k, v in x.items():
                    recursive_print(v, prefix=prefix + '-' + k)
            else:
                print(f"{prefix}: {x}")

        sample_cnt = 0
        self.current_iteration += 1
        for q in self.stat_queues:
            for _ in range(32):
                try:
                    stat = q.get_nowait()
                    sample_cnt += self.batch_size * self.n_step
                except queue.Empty:
                    break
        # worker_bs = self.batch_size // SURREAL_WORLD_SIZE
        # for i, q in enumerate(self.batch_queues):
        #     worker_batch = recursive_apply(batch, lambda x: None if x is None else x[i * worker_bs: (i+1) * worker_bs] if x.shape[0] == self.batch_size else x[:, i * worker_bs: (i+1) * worker_bs])
        #     self.sockets[i].send(pickle.dumps(worker_batch))
        # for socket in self.sockets:
        #     tensorplex_update_dict = pickle.loads(socket.recv())
