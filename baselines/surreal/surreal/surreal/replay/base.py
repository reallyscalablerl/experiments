...
class Replay:
    ...
    def __init__(self,
        ...
        self._sampler_servers = [ZmqServer(
            host='localhost',
            port=str(int(sampler_port) + rank * 100),
            bind=False) for rank in range(SURREAL_WORLD_SIZE)]
        self._sampler_server_threads = None
    ...
    def start_threads(self):
        ...
        self._sampler_server_threads = [s.start_loop(
            handler=self._sample_request_handler) for s in self._sampler_servers]

    def join(self):
        self._collector_server.join()
        [t.join() for t in self._sampler_server_threads]
        if self._has_tensorplex:
